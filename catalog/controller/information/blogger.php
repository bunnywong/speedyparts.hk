...
class ControllerInformationBlogger extends Controller {
	...
	public function index() {
		...
		$language = $this->model_megnor_blogger->getLanguageByCode($this->session->data['language']);

		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_megnor_blogger->addComment($this->request->get['blogger_id'], $this->request->post, $language['language_id']);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('information/blogger', '', 'SSL')
		);

		$data['breadcrumbs'][] = array(
			'text' => $blog_info['title'],
			'href' => $this->url->link('information/blogger', 'blogger_id=' . $this->request->get['blogger_id'], 'SSL')
		);

		$data['heading_title'] = $blog_info['title'];
		
		$data['date'] = date('j', strtotime($blog_info['date_added'])).' '.date('F', strtotime($blog_info['date_added'])).''.'<div class="year">'.date('Y', strtotime($blog_info['date_added'])).'</div>';
		
		$module = $this->model_megnor_blogger->getModule($blog_info['module_id']);
		...
		$data['text_login_required'] = sprintf($this->language->get('text_login_required'), $this->url->link('account/login', '', 'SSL'), $this->url->link('account/register', '', 'SSL'));
		...
		$blog_comments = $this->model_megnor_blogger->getBlogComments($this->request->get['blogger_id'], $language['language_id']);
		...
		$data['action'] = $this->url->link('information/blogger', 'blogger_id=' . $this->request->get['blogger_id'], 'SSL');
		...
		if (!$this->customer->isLogged()) {
			$this->session->data['redirect'] = $this->url->link('information/blogger', 'blogger_id=' . $this->request->get['blogger_id'], 'SSL');
		}
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/information/blogger.tpl')) {
			$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/information/blogger.tpl', $data));
		}
		}
	...
	public function blogs() {
		...
		$blogs = $this->model_megnor_blogger->getBlogs();

		foreach ($blogs as $blog) {
			$module_blog_data = array();

			$module = $this->model_megnor_blogger->getModule($blog['module_id']);

			$setting = json_decode($module['setting'],true);

			if (!empty($setting['status']) && $setting['status']) {
				$module_blogs = $this->model_megnor_blogger->getBlogsByModule($blog['module_id']);
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/information/blogger_blogs.tpl')) {
			$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/information/blogger_blogs.tpl', $data));
		}
	}
}